...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($buletin['name'])) {
        ...
        if ($uploadOk == 0) {
            ...
            header("Location: editare_angajat.php?id=$id");
            exit();
        } else {
            ...
            if (move_uploaded_file($buletin["tmp_name"], $target_file)) {
                ...
                $sql = "UPDATE angajati SET nume=?, prenume=?, numar_telefon=?, email=?, functie=?, salariu=?, buletin=? WHERE id=?";
        ...
        $sql = "UPDATE angajati SET nume=?, prenume=?, numar_telefon=?, email=?, functie=?, salariu=? WHERE id=?";
        $stmt = $conn->prepare($sql);
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['message'] = "Eroare la actualizarea angajatului: " . $stmt->error;
        header("Location: editare_angajat.php?id=$id");
        exit();
    }
